...
impl XdpDispatcher {
    ...
    fn load_xdp_dispatcher_with_exts<Ext: BorrowMut<Extension>>(
        ...
        let mut dispatcher_bpf = EbpfLoader::new()
            ...
            .load(&AGAVE_XDP_DISPATCHER_EBPF_PROGRAM)?;
    ...
    fn dispatcher_lock(if_index: u32) -> Result<NamedLock> {
        Ok(NamedLock::with_path(format!(
            "/tmp/.dispatcher_{if_index}_lock"
        ))?)
    }

    pub fn new_with_programs<'a>(if_index: u32, xdp_flags: XdpFlags, bpfs: Vec<&'a mut EbpfPrograms<'a>>) -> Result<Self> {
        ...
        for bpf in bpfs.iter() {
            ...
            if total_programs > MAX_PROGARMS {
                return Err(std::io::Error::other("program count more than 10").into());
            }
        }
